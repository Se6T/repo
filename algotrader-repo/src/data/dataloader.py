...
def get_dataset(filename: str, filepath: str = "../../data/interim") -> pd.DataFrame:
...
class TradingDataset(Dataset):
    def __init__(
        ...
        # self.df = self.df.drop(columns='Date') Date is the index
        ...
        self.length = len(self.df) - seq_length - self.output_dim
    ...
    def __getitem__(self, idx):
        ...
        X = torch.tensor(np.diff(X, axis=0))  # only handing over the difference between subsequent values. 
...
def get_X_y() -> tuple[np.array, np.array]:
    pass
